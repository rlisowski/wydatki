...
def _ofc(width, height, url,  base="/", set_wmode_transparent=false)
  puts url.inspect
  url = CGI::escape(url)
  puts url.inspect
  out = []
...
def _ofc_str(width, height, data, base, set_wmode_transparent=false)
  data = data.gsub("\r",'').gsub("\n",'').gsub('&&','&') #CGI::escape(data)
  ...
  #    if el.size>0
  #      ary = el.split('=')
  #      out << 'so.addVariable("'+ary[0]+'", "' + ary[1] + '");' unless ary[0].nil? || ary[1].nil?
  #    end
  #  end
  #  out << 'so.addVariable("data", getQueryParamValue("'+url+'"));'

  out << 'so.addParam("wmode", "transparent");' if set_wmode_transparent
  out << 'so.addParam("allowScriptAccess", "sameDomain");'
  out << 'so.addVariable("variable1", "value1");'
  out << 'so.addVariable("variable2", "value2");'
  out << 'so.addVariable("variable3", "value3");'
  out << 'so.write("' + div_name + '");'
  out << '</script>'
  return out.join("\n")
end

def open_flash_chart_object_str(width, height, data, base='/', set_wmode_transparent=false)
  return _ofc_str(width, height, data, base , set_wmode_transparent)  
end

def open_flash_chart_object(width, height, url, base='/', set_wmode_transparent=false)
  return _ofc(width, height, url, base, set_wmode_transparent)
end
